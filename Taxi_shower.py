...
# log_agent_rewards_list = np.array(log_agent_rewards_list[0:4])
...
log_agent_rewards_list = []
for i in range(5):
    path = log_paths[i]  # INDL

    temp_log_agent_rewards_list = []
    with open(path + f'train_history_{0}.pkl', 'rb') as f:
        train_history = pickle.load(f)
        temp_log_agent_rewards_list.append(train_history['agent_rewards'])
    # log_agent_rewards_list.append(temp_log_agent_rewards_list)
...
# print(np.array(temp_log_agent_rewards_list).shape)
# QMAX = temp_log_agent_rewards_list[0][:5000]
# log_agent_rewards_list.append(QMAX)

# %%
len(log_agent_rewards_list[-1])
# %%
np.array(log_agent_rewards_list).shape
...
fig, axs = plt.subplots(1, 1, figsize=(6, 6))  # 创建1行3列的子图
agents = ['Agent Avg']
...
for i in range(1):
    for j in range(len(log_paths)):  # 循环遍历每个test
        data = copy.deepcopy(log_agent_rewards_list[j, :])
        ...
        if add_sd:
            ...
            # data = smooth(data, 2)
            axs.plot(range(window_size, len(data) + window_size), data, label=legends[j])  # 绘制每个agent在每个test的数据

    axs.set_title(agents[i])  # 设置子图标题
    axs.set_xlabel('Episodes', fontsize=15)  # 设置x轴标签
    axs.set_ylabel('Running Average Rewards', fontsize=15)  # 设置y轴标签
    axs.grid(True)
    if not add_sd:
        axs.legend()  # 显示图例
...
for i in range(1):
    ...
    # 找到这个客户端的最小值
    for j in range(len(log_paths)):  # 循环遍历每个test
        data = log_agent_rewards_list[j, :]  # 取第j个算法的第i个agent的数据
