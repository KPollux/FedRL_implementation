...
def plot_ax(
    ...
    for experiment_index in range(data_arrays.shape[0]):
        y = data_arrays[experiment_index, :, agent_index, :].mean(axis=0)
        y = smooth(y, radius=smooth_radius)
        color = COLORS[experiment_index % len(COLORS)]
        ax.plot(x, y, color=color)
        if shaded_std:
            y_shaded = data_arrays[experiment_index, :, agent_index, :].std(axis=0)
            y_shaded = smooth(y_shaded, radius=smooth_radius)
