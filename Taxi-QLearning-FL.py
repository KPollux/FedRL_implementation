...
class FLServer:
    def __init__(self, aggregation_method='QAvg'):
        self.agent_q_tables = []
        self.global_q_table = np.zeros([500, 6])
...
server = FLServer(aggregation_method='QDynamicAvg')
agents = [Agent(gym.make("Taxi-v3").env, server) for _ in range(NUMBER_OF_AGENTS)] 
