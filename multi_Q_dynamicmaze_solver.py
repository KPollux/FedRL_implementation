...
def main(share_params=False, FL=False, role=False, share_memory=False, FLMax=False, FLdev=False, onlyA=False,
    ...
    # Create environment
    n_agents = 3
    reward_dict=[{'step': -1, 'collision': -10, 'goal': 50, 'heuristic': True},
    ...
    for i_episode in range(EPISODES):
        ...
        #             Q_tables[idx] = copy.deepcopy(global_Q)
            ...
            if i_episode % FL_LOCAL_EPOCH == 0 and i_episode != 0:
                # # 分别计算每个Agent的Q表与全局Q表的差值，得到DeltaQ
                # delta_Qs = [Q_tables[i] - global_Q for i in range(env.n_agents)]
                ...
                # norm_delta_Qs = []
                # for i in range(env.n_agents):
                #     tau_clients.append(np.sum(agent_paths_length[i][-FL_LOCAL_EPOCH]))  # 本次更新的轨迹总长度
                #     norm_delta_Qs.append(delta_Qs[i] / tau_clients[i])      


                # d_total_round = copy.deepcopy(global_Q)
                # d_total_round = d_total_round * 0.0
                    
                # for i in range(env.n_agents):
                #     d_para = norm_delta_Qs[i]
                #     d_total_round += d_para / 2

                # coeff = 0.0
                # for i in range(env.n_agents):
                #     coeff = coeff + tau_clients[i] / 2

                # global_Q += coeff * d_total_round
    ...
    #     prefix = 'Q_learning_M1720_5flod_0Reward_'
    ...
    #     floder_name += '{}LE_'.format(FL_LOCAL_EPOCH) if 'FL' in floder_name else ''
...
floder_name = None
for n in range(1):
    train_history, policy_nets, floder_name, deltas_dict = main(FLdev=True,
                                                dynamic=True,
                                                floder_name=floder_name, n_times=n)
