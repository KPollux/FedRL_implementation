...
def plot_confidence_interval(ax, data, agent_index, xlabel, ylabel, window_size=100):
    """
    This function plots the confidence interval of the moving average of data.

    Parameters:
    ax (matplotlib.axes._axes.Axes): The axes of the subplot.
    data (numpy.ndarray): The data array with shape (num_experiments, num_repeats, num_agents, num_episodes).
    agent_index (int): The index of the agent.
    xlabel (str): The label of x axis.
    ylabel (str): The label of y axis.
    window_size (int, optional): The window size of moving average. Defaults to 100.

    Returns:
    None

    """
    num_experiments, num_repeats, _, num_episodes = data.shape
    
    # make sure num_episodes is greater than or equal to window_size
    assert num_episodes >= window_size, "num_episodes must be greater than or equal to window_size"
    
    x = np.arange(window_size, num_episodes + 1)

    colors = COLORS[:num_experiments]

    for i in range(num_experiments):
        ma_data = np.apply_along_axis(moving_average, 1, data[i, :, agent_index, :], window_size=window_size)
        y = ma_data.mean(axis=0)
        yerr = ma_data.std(axis=0)
        ax.plot(x, y, color=colors[i])
        ax.fill_between(x, y - yerr, y + yerr, color=colors[i], alpha=0.2)

    ax.set_xlabel(xlabel)
    ax.set_ylabel(ylabel)


data = copy.deepcopy(log_agent_rewards_list)

fig, axes = plt.subplots(1, 3, figsize=(15, 5))

for i in range(3):  # 3 agents
    plot_confidence_interval(axes[i], data, i, xlabel='Episode', ylabel='Reward', window_size=6400)
