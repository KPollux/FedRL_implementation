...
class FLServer:
        ...
        async with self.condition:
            while not all(agent.done for agent in self.agents):
                ...
                if self.cfg['FL']:
                    ...
                    # tic = time.time()
                    for agent in agents:
                        self.update_table(agent.V, i, 'V')
                        self.update_table(agent.C, i, 'C')
                    # toc = time.time()
                    # print(f'Upload time: {toc-tic}')
                    # 聚合
                    # tic = time.time()
                    self.aggregate_all_tables()
                    # toc = time.time()
                    # print(f'Aggregate time: {toc-tic}')
                    # 下载
                    # tic = time.time()
                    for agent in agents:
                        agent.V = copy.deepcopy(self.distribute_V())
                        agent.C = copy.deepcopy(self.distribute_C())
                    # toc = time.time()
    ...
    def aggregate_table(self, table_type='V'):
        NUMBER_OF_AGENTS = self.cfg.NUMBER_OF_AGENTS
...
class Agent:
    def __init__(self, env, alpha, gamma, cfg, condition, id):
        self.condition = condition
        self.ready = False
        ...
        self.t = 0 # 时间步
        self.cfg = cfg
        self.max_t = self.cfg['MAX_LOCAL_STEP'] # 最大时间步
    ...
    async def MAXQ_0(self, i, s):
        ...
        if self.done:
            ...
            # print(f'Agent {self.id}: episode done in {self.t}')
        self.done = False
        if self.is_primitive(i):
            ...
            if self.t > self.max_t:
                self.done = True
            # print(f'Agent {self.id}: update V step {self.t}')
            # print(f'Agent {self.id}, Action {i}, Done {self.done}， Ready {self.ready}, t {self.t}: wait for server...')
            if self.t % self.cfg['FL_LOCAL_STEP'] == 0 or self.done:
                async with self.condition:
                    self.ready = True  # Set ready status
                    self.condition.notify_all()  # Notify the server
                    # print('Notify the server')
                    await self.condition.wait_for(lambda: not self.ready)  # Wait for the server
            return 1
        elif i <= self.root:
            count = 0
            ...
            while not self.is_terminal(i, self.done):
                ...
                # print(f'Agent {self.id}: update C step {self.t}')
                count += N
                s = self.new_s
            return count
...
cfg.ShareQ = False
cfg.FL_LOCAL_STEP = 8
cfg.MAX_LOCAL_STEP = 128
cfg.NUMBER_OF_AGENTS = 2
cfg.training_episodes = 5000
cfg.aggregation_method = 'QAll'  # QDynamicAvg

condition = asyncio.Condition()
agents = [Agent(env(maze_cross, fickle=True, allow_collision=False), alpha, gamma, cfg, condition, id) for id in range(cfg.NUMBER_OF_AGENTS)] 
server = FLServer(env(maze_cross, fickle=True, allow_collision=False), agents, cfg, condition)
...
# %%
# e = env(maze_cross, fickle=True)
# e.reset()
# print(e.render())

# %%
async def train():
    await asyncio.gather(server.run(), *(agent.MAXQ_0(10, agent.env.s) for agent in agents))
# %%
...
if floder_name is None:
    prefix = 'Taxi5_Fickle_no_collision_MaxQ-0_'

    floder_name = prefix  # M17_20_all_delta_

    # floder_name += 'dynamic_' if dynamic else ''
    if cfg['FL']:
        floder_name += 'FL_' if cfg['aggregation_method']=='QAvg' else ''
        ...
        floder_name += 'FLDynamicAvg_' if cfg['aggregation_method']=='QDynamicAvg' else ''
...
a = np.random.rand(11, 1280)
b = np.random.rand(11, 1280)
