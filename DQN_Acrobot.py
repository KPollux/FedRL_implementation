...
LOCAL_EPISODES = 8
all_time_steps = 300_000

# DDPG 中使用的噪声对象
envs = [gym.make('Acrobot-v1') for _ in range(agent_num)]
...
if mode == 'INDL':
    for _ in trange(math.ceil(all_time_steps/LOCAL_EPISODES)):
    ...
    for i_communication in trange(math.ceil(300_000/LOCAL_EPISODES)):
        ...
        EPS_DECAY = math.ceil(300_000/LOCAL_EPISODES) * 0
...
# 绘制历史平均曲线
# agent_num = 3
# plt.plot(train_history['0']['agent_rewards'], label=f'agent_0')
# %%
for idx in range(agent_num):
    idx = str(idx)
    # 在每一个step上求历史平均值
