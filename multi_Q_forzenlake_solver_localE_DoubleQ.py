...
def main(share_params=False, FL=False, role=False, share_memory=False, FLMax=False, FLdev=False, onlyA=False,
    ...
    # Create Q-tables
    Q_tables = [np.zeros([num_states, num_actions]) for _ in range(2*env.n_agents)]
    if share_params:
        Q_tables = [Q_tables[0], Q_tables[1]]*env.n_agents
    ...
    for i_episode in range(EPISODES):
        ...
        for t in count():
            ...
            for idx in range(env.n_agents):
                ...
                action = np.argmax(Q_tables[2*idx][state_index]) if np.random.uniform() > EPSILON else np.random.choice(env.action_space)
                ...
                if onlyA and idx!=0:
                    ...
                    if np.random.uniform() < 0.5:  # Half the time update Q1, the other half Q2
                        Q_tables[2*idx][state_index, action] = \
                            Q_tables[2*idx][state_index, action] + LR * (reward + GAMMA * Q_tables[2*idx+1][next_state_index, np.argmax(Q_tables[2*idx][next_state_index])] - Q_tables[2*idx][state_index, action])
                    else:
                        Q_tables[2*idx+1][state_index, action] = \
                            Q_tables[2*idx+1][state_index, action] + LR * (reward + GAMMA * Q_tables[2*idx][next_state_index, np.argmax(Q_tables[2*idx+1][next_state_index])] - Q_tables[2*idx+1][state_index, action])
    ...
    if floder_name is None:
        ...
        if floder_name == prefix:
            floder_name += 'Independent_DoubleQ_'
