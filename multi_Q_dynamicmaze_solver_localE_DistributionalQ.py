...
def main(share_params=False, FL=False, role=False, share_memory=False, FLMax=False, FLdev=False, onlyA=False,
    ...
    V_MIN = -10  # Minimum value of distribution
    V_MAX = 10  # Maximum value of distribution

    # Create Q-tables, now a 3D array with third dimension for atoms
    Q_tables = [np.zeros([num_states, num_actions, N_ATOMS]) for _ in range(env.n_agents)]
    if share_params:
        Q_tables = [Q_tables[0] for _ in range(env.n_agents)]
    global_Q = np.zeros([num_states, num_actions, N_ATOMS])
    ...
    for i_episode in trange(EPISODES):
        ...
        for t in count():
            ...
            for idx in range(env.n_agents):
                ...
                action = np.argmax(np.sum(np.multiply(Q_tables[idx][state_index], support), axis=1)) if np.random.uniform() > EPSILON else np.random.choice(env.action_space)
                # print(action)
                EPSILON *= 0.001
                if EPSILON < 0.1:
                    EPSILON = 0.1
                ...
                if onlyA and idx!=0:
                    ...
                    # Calculate target atoms for all possible next actions
                    target_atoms_all = [np.clip(reward + GAMMA * support, V_MIN, V_MAX) for _ in env.action_space]
                    
                    # Compute the projection indices for all possible next actions
                    b_j_all = [(target_atoms - V_MIN) / delta_z for target_atoms in target_atoms_all]
                    l_all = [np.floor(b_j).astype(int) for b_j in b_j_all]
                    u_all = [np.ceil(b_j).astype(int) for b_j in b_j_all]
                    
                    # Distribute probability of j-th atom for all possible next actions
                    m_prob = Q_tables[idx][state_index, action].copy()
                    m_prob_next_all = [np.zeros_like(m_prob) for _ in env.action_space]  # Initialize new blank distributions
                    
                    for a in range(len(env.action_space)):
                        for j in range(N_ATOMS):
                            m_prob_next_all[a][l_all[a][j]] += m_prob[j] * (u_all[a][j] - b_j_all[a][j])
                            m_prob_next_all[a][u_all[a][j]] += m_prob[j] * (b_j_all[a][j] - l_all[a][j])
                        
                        Q_tables[idx][state_index, a] = m_prob_next_all[a]  # Replace original distributions with new projected ones
                state = next_state
